...
function updateAdvisor() {
  ...
  else if (markets > 0 && food > yearlyFoodNeed * 4 && gold < 500) {
    // много еды, мало золота — пора продавать
    msg = "Склад забит едой, а казна — молью. Продайте часть запасов на рынке, пока крысы не открыли там собственный ресторан.";
  } else if (markets > 0 && food < yearlyFoodNeed && gold > 1000) {
    // мало еды, много золота — пора покупать
    msg = "Еды не хватает, золото валяется без дела. Рынок уже стоит — купите пайки, пока народ не пришёл за вашим пайком.";
  } else if (markets > 0 && game.iron > 100 && gold < 300) {
    // избыток железа
    msg = "Железа у нас как грязи, а золота — как совести у сборщиков налогов. Самое время продать немного руды и превратить её в звонкую монету.";
  } else if (markets === 0 && (food > yearlyFoodNeed * 3 || game.iron > 80)) {
    // есть ресурсы, но нет рынков
    msg = "У нас приличные запасы, но ни одного рынка. Мы буквально сидим на богатстве и смотрим на него. Постройте рынок — пусть хотя бы кто-то начнёт на этом зарабатывать.";
  }
  ...
  // --- БЛИЗКАЯ ПОБЕДА / СТАБИЛЬНОСТЬ ---
    ...
    msg = "Империя развивается стабильно. Продолжайте аккуратно крутить налоги, рационы и торговлю, и, возможно, вы умрёте не от вил, а от старости. Для нашего региона это успех.";
  }
